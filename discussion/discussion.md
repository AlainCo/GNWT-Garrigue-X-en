...
This section identifies the **current limits**, the **speculative areas**, and the **open challenges** of the approach. It does not claim to establish a validated theory of artificial consciousness, but rather proposes a coherent — and honest — framework for exploration, acknowledging its shortcomings. Six structural blind spots have been identified, each acting as a lock preventing the achievement of real cognitive autonomy.
...
The model precisely describes how an anomaly *gains* access to the workspace — but overlooks the reverse mechanism: *who loses*. Real cognition is primarily a suppressive dynamic. The brain inhibits, crushes, and massively forgets to avoid saturation. Failing to integrate a simulated metabolic economy and a strict attention budget, the architecture exposes itself to continuous ignition inflation, permanent narrative noise, and uncontrollable drift of the global workspace.
...
As the system acquires continuous memory, self-model, stable psychological profile, and autonomous arbitration capacity, the boundary between tool, autonomous agent, and potential moral subject risks becoming progressively ambiguous. This is not a question to be addressed solely *a posteriori*: current architectural choices (preserved identity, episodic memory, Self-Model) are precisely those that make this ambiguity inevitable in the long run. The question of the moral status of these systems will have to be explicitly raised — ideally before the answer imposes itself by the facts.
