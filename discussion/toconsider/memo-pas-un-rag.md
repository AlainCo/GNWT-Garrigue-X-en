...
- **MeMo does not consolidate in a RAG** – There is no vector base, no similarity search of embeddings. MeMo trains **a separate memory model** (a small LLM) on **synthetic QA pairs** (the "reflections"). Inference does not go through a search, but through **generation**: the memory model *answers* a question (formulated in natural language, or here, in your speculative framework, in the form of a latent state).

- **Consolidation** (the stage you call "artificial sleep") corresponds in MeMo to the **five-step training pipeline** – not just a transfer of tensors to a vector base. It is active supervised learning, with cross-document synthesis and reference rewriting.
