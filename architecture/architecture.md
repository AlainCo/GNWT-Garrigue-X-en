...
Each conscious level also integrates a Self-Model (metacognition), an Attention Scheduler (attention budget), and, during sleep phases, a Φ-estimator (causal integration) – see in the sections [Key Concepts and Theoretical Foundations](../concepts/concepts.md).
...
flowchart TD
    subgraph N5 ["N=5 : Group (Officers + JEPA-L)"]
        V5["Validation of the Summary<br>• Variance verification<br>• Collapse detection<br>• Uncertainty score"]
...
**Communication Rule:** An officer only transmits to the shared workspace what has passed their personal ignition threshold. Like a team of seasoned professionals who know, respect, and know how to behave — they don't speak at every micro-event, they speak when it matters.
...
**1. N=0/N=1:** A missile fragment damages the right nozzle. The PID increased by the MLP nano instantly modifies the injection angles in **4 milliseconds** to prevent engine shutdown. No signal is sent back — it's handled locally, below the RPT threshold.

**2. N=2/N=3:** The engine's Mamba model records a growing anomaly. Its local RPT loops run, attempt to consolidate an assessment. After stabilization, they generate a **Vector Ignition Summary**: *[propulsion_anomaly | severity=0.73 | type=thrust_asymmetry | workaround_available=true]*. No raw data dump — a compressed semantic vector.
...
- the higher level can request an **enriched Ignition** (fallback)
